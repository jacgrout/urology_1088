...
This project has identified that the underlying level of need in the Midlands varies between sub-ICBS and condititons. Prevalences for Benign Prostatic Hyperplasia were found to range from 4.77% to 7.19% across the sub-ICBs of the Midlands region. For Overactive Bladder the prevalence was found to vary from 9.44% to 11.51% across the region. Post prostatectomy incontinence prevalence was found to be much lower in general than other conditions varying from 0.22 to 0.70 per 1000 list size across the sub-ICB's in the region.

Activity to need ratios and the consequent activity gaps were found to vary considerably for inpatient activity with activity in some sub-ICBs needing to increase as much as one and a half times if they were to match the sub-ICBs with the highest activity to need ratios.

Levels of prescribing were also found to vary across the region and it's possible that different treatment options, such as a greater emphasis on primary care management are evident where some sub-ICBs show high activity to need ratios for prescribing with low activity to need ratios for inpatient activity (Leicester City being one such example).

Treatment via medication is not the only option other than surgical intervention and there can be considerable side effects. The options offered, whether they are non-surgical interventions, medication, surgery etc are impacted by initial diagnosis, comorbidities and preferences of patients, general practitioners and consultants. This leads to a complex and varied picture of activity across the region.

```{r}
#(The medications are not particularly effective and when a patient is on multiple #other medications its not necessarily a good idea to go on these drugs. #Consultants say they want a patient on a it but GP's then discuss with patient and #when they learn of side effects etc then can decide not to (esp if elderly). Good #treatment options are unfortunately limited.) - Matt
```
...
tmap_mode("plot") #interactive maps = "view"
...
As with Benign Prostatic Hyperplasia there is an activity gap for Overactive Bladder and in order to attempt to quantify this activity gap, the activity to need ratio for the sub-ICB with the highest ratio has been applied to all other sub-ICBs and an estimated number of inpatient spells calculated as well as the % increase in activity that this represents. The table below shows that Leicester City (04C) sub-ICB would need a 163% increase in activity to increase its activity to need ratio to the same as Bassetlaw (02Q) sub-ICB.
...
bph_activity_icb_drugs_mid |>
  ...
  fmt_number(columns = 2:2,
             decimals=2) |>
  fmt_number(columns = 7:7,
             decimals=2) |>
    tab_header(
    title = 'Prescription Items Activity To Need Ratio by Sub_ICB'
  ) |>
  tab_source_note(source_note = "Source: OpenPrescribing.net, Bennett Institute for Applied Data Science, University of Oxford, 2023")
```
...
subicb_bph_ratio_drugs_mid <- subicb_bph_ratio |> 
  left_join(bph_activity_icb_drugs_mid)


tm_shape(subicb_bph_ratio_drugs_mid) +
  tm_fill("presc_anr",title="Activity/Need Ratio BPH",style = "pretty", palette = "-viridis") +
  tm_borders("grey25", alpha=.5) +
  tm_shape(ppi_providers_sf) + 
  tm_dots(col = "short_role",size = 0.05, title = "Providers")+
  tm_basemap(server = c('OpenStreetMap'))
...
subicb_oab_ratio_drugs_mid <- subicb_oab_ratio |> 
  left_join(oab_activity_icb_drugs_mid)


tm_shape(subicb_oab_ratio_drugs_mid) +
  tm_fill("presc_anr",title="Activity/Need Ratio OAB",style = "pretty", palette = "-viridis") +
  tm_borders("grey25", alpha=.5) +
  tm_shape(ppi_providers_sf) + 
  tm_dots(col = "short_role",size = 0.05, title = "Providers")+
  tm_basemap(server = c('OpenStreetMap'))
...
```{r}
ppi_activity_icb_drugs_mid |>
  gt() |> 
  cols_hide(activityneedratio) |>
  cols_label(
    sub_icb_location_code = md("Sub-ICB"),
    ppiprevnumtotal = md("Prevalence"),
  #  list_size_ppi = md("List Size"),
    num_ppi_spells_sum = md("Spells"),
    items_total = md("Items"),
    presc_anr = md("Activity to Need Ratio")
  ) |>
  fmt_number(columns = 6:6,
             decimals=2) |>
    tab_header(
    title = 'Prescription Items Activity To Need Ratio by Sub_ICB'
  ) |>
...
subicb_ppi_ratio_drugs_mid <- subicb_ppi_ratio |> 
  left_join(ppi_activity_icb_drugs_mid)


tm_shape(subicb_ppi_ratio_drugs_mid) +
  tm_fill("presc_anr",title="Activity/Need Ratio PPI",style = "pretty", palette = "-viridis") +
  tm_borders("grey25", alpha=.5) +
  tm_shape(ppi_providers_sf) + 
  tm_dots(col = "short_role",size = 0.05, title = "Providers")+
  tm_basemap(server = c('OpenStreetMap'))
...
Table by GP Practice of Inpatient Activity to Need Ratios
...
ppi_gp <- ICBTOPPI |>
  filter(icb22 %in% midlands_icbs) |>
  select(org_code, practice_name, ppiprev,list_size_ppi_total, num_ppi_spells,new_sub_icb_location_name, icb22nm) |>
  mutate(anr = num_ppi_spells/ppiprev)

gp_all <- bph_gp |>
  rename(bph_anr = anr) |>
  left_join(select(oab_gp, org_code, oabprevnumtotal, list_size_oab_total, num_oab_spells, anr)) |>
  rename(oab_anr = anr) |>
  left_join(select(ppi_gp, org_code, ppiprev, list_size_ppi_total, num_ppi_spells, anr)) |>
  rename(ppi_anr = anr)


  gp_all_table <- select(gp_all, new_sub_icb_location_name,org_code, practice_name, list_size_bph_total, bphprevnumtotal, num_bph_spells, 
                       bph_anr, oabprevnumtotal, num_oab_spells,oab_anr, ppiprev, num_ppi_spells, ppi_anr) |>  
  gt(groupname_col = "new_sub_icb_location_name") |>
  cols_label(
    ...
    bph_anr = md("Activity To Need Ratio"),
    oabprevnumtotal = md("Prevalence"),
    num_oab_spells = md("Spells"),
    oab_anr = md("Activity To Need Ratio"),
    ppiprev = md("Prevalence"),
    num_ppi_spells = md("Spells"),
    ppi_anr = md("Activity To Need Ratio")
  ) |> 
  tab_spanner(
  ...
  fmt_number(columns = 5:5,
             decimals=2) |>
  # fmt_number(columns = 5:6,
  #            decimals=0) |>
  fmt_number(columns = 7:7,
             decimals=3) |>
  fmt_number(columns = 8:8,
             decimals=2) |>
  # fmt_number(columns = 9:10,
  #            decimals=0) |> 
  fmt_number(columns = 10:10,
             decimals=3) |>
  # fmt_number(columns = 12:12,
  #            decimals=0) |> 
   fmt_number(columns = 13:13,
            decimals=3) |> 
      tab_style(
    style = cell_text(color = "black", weight = "bold"),
    locations = list(
  ...
  tab_header(
    title = 'Activity To Need Ratio by GP practice'
  ) |>
  tab_source_note(source_note = "Data: SUS & Patients Registered at a GP Practice (NHS Digital)")
...
Below are details of all relevant procedure codes and medications:
